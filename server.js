...
function checkWord(socket, words) {
    const signature = arguments.callee.name;
    debugConsole(CATEGORY.FUNCTION, signature);
...
async function createScrabbleRoom(socket, clientID, playerCount) {
    ...
    try {
        ...
        // let columns = [KEYS.REMAINING_LETTERS,
        // KEYS.PLAYER_COUNT,
        // KEYS.WHOSE_TURN,
        // KEYS.ROOM_CODE,
        // KEYS.IP1, KEYS.IP2, KEYS.IP3, KEYS.IP4,
        // KEYS.POINTS1, KEYS.POINTS2, KEYS.POINTS3, KEYS.POINTS4,
        // KEYS.DOCK1, KEYS.DOCK2, KEYS.DOCK3, KEYS.DOCK4,
        // KEYS.PASS1, KEYS.PASS2, KEYS.PASS3, KEYS.PASS4];

        // let values = ['',
        //     4,
        //     1,
        //     newRoomCode,
        //     clientID, 'testplayer2', 'testplayer3', 'testplayer4',
        //     381, 500, 200, 401,
        //     'T,E,S,T', 'Q', 'A,B,E', 'A,R,C,Z',
...
async function endTurn(mode, socket, roomCode, clientID, points, tilesToSwap, boardState, dock, wordsSubmitted, pointsEarned) {
    ...
    try {
        ...
        if (mode === 'submit') {
            sendMessage(roomCode, clientID, false, `<b>${clientID}</b> submitted "${wordsSubmitted}" for ${pointsEarned} points`, recipients);
        } else if (mode === 'pass') {
            sendMessage(roomCode, clientID, false, `<b>${clientID}</b> passed (0 points)`, recipients);
        } else if (mode === 'swap') {
            sendMessage(roomCode, clientID, false, `<b>${clientID}</b> swapped some letters (0 points)`, recipients);
        }

        // checking if game should end
        let passValues = [record[KEYS.PASS1], record[KEYS.PASS2], record[KEYS.PASS3], record[KEYS.PASS4]];
        passValues[playerNumber - 1] = newPassValue;
...
async function endGame(roomCode, clientID) {
    ...
    try {
        ...
        const longestIP = ips.reduce((a, b) => (a.length > b.length ? a : b), '');
...
async function deleteOldRecords() {
    ...
    try {
        ...
        thresholdDate.setDate(thresholdDate.getDate() - gameExpirationDays);
...
function getUpdatedGameData(mode, record, playerNumber, points, tilesToSwap, boardState, dock) {
    ...
    const newPassValue = (mode === 'pass') ? ((record[KEYS.REMAINING_LETTERS].length === 0) ? true : record[passKey]) : false;
